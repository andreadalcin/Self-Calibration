function [] = displayClustersCircle(X,F,opts, epsi)
...
F = prune_small_clust(F,3);
M = recover_circle(X,F);
...
for j  = 1:numSeg
    ...
    if(autoEpsi)
        resi = res_circle(X(:,F==j), M(:,j));
        bandWidth = x84_(resi,1);
    else
        bandWidth = epsi;
    end
    displayAnularBand(X(:,F==j), M(:,j),bandWidth,cmap(j,:));
end
