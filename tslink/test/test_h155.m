...
% To be implemented modelType = subspace 4 or subspace 5
% puoi provare a guardare in fun/model_spec:
% - fit_subspace
% - res_subspace
% - recover_subspace deve essere implementato
% - sarebbe utile avere un check sul non essere degenere per un sottospazio
% (quando fai l'svd il numero di valori singolari più grandi di una soglia è la dimensione
% del sottospazio)
% anche in h155Tools puoi trovare roba che forse è utile
for modelType = {'plane'}
    close all;
    modelType = modelType{:};
    % ho cambiato la struttura dei folder, ora ho una cartella Develop
    % con 3 sottocartelle Data, Codes, Results
    % Develop
    % |_ Data
    % |_ Codes
    %         |_ tslink
    % |_ Results
    %         |_ tslink
    % se ti va possiamo salvare tutto in Results/tslink/
    % se no lasciamo così, tanto cazzocene
    saveDir = ['results/traffic/',dt,'/',modelType,'/trial',id,'/'];
    
    %%% Questi di seguito saranno da cambiare!
    sigma = 0.011; % sarà da tunare immagino..
    cardmss = 3; %dipenderà dalla dimensione del sottospazio cardmss = dim +1;
    nEpsis = 119;
    ...
    %%%
    %% view pairs names
    % i 155 nomi del dataset li puoi caricare da dei .mat 
    % che trovi in una sottocartella dei Dati h155/nameList
    % i nomi sono così divisi
    %  CB3 sequenze di tipo scacchiera con 3 moti
    %  CB2 sequenze di tipo scacchiera con 2 moti
    %  Traffic3 sequenze di tipo traffico con 3 moti
    %  Traffic2 sequenze di tipo traffico con 2 moti
    %  O3 tutto il resto con 3 moti
    %  O2 tutto il resto con 2 moti
    % 
    % Forse vale la pena fare uno script per ogni tipo di dataset così li
    % lanciamo in parallelo.
    % Facciamo finta che questo sia quello per CB2
    
    data_folder = '../../Data/h155';
    temp = load([data_folder,'nameList/nameCB3.mat'],'nameCB3');
    nameDataset = temp.nameCB3; % da verificare che funzioioni
    
    
    % Ti lascio commentati un po' di nomi nel caso siano utili
    % three motions
...
%     nameDataset{51} ='arm';
    ...
    for s = 1:nSeq
        ...
        % puoi settare la dimensione di dove vuoi proiettare i dati
        % proviamo con 9 5 o 4
        ...
        for iter = 1:maxIter
            %%
            % Qui ci sarà da adattare il sampling
            ...
            for e = 1:nEpsis
                ...
                % questa dovrebbe funzionare così
                ...
                t2(s,e,iter) = toc(t2s);
                t3s = tic;
                % e' un po' una porcata ma conviene farci un tslnkGric
                % apposta per h155 in modo da non rishciare di rompere le cose 
                % vecchie e garantirci la retro
                % compatibilità sui dati... anche io ho fatto apposta un tslink_moseg
                % per il kitti3D poi passata la tempesta
                % unifichiamo bene tutto
