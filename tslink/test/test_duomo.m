%%
cd(fileparts(which('test_duomo.m')));
addpath(genpath('../'))
%% Options
clean;
tstart = tic;
...
saveDir = ['results/pointclouds/duomoPisa/',dt,'/trial',id,'/'];
...
gricParam.lambda2 = 2;
%%
pc = pcread('duomoalignedcleaned3.ply');
%pc =pcdenoise(pc);
%pc = pcdownsample(pc,'random',0.025);
pc = pcdownsample(pc,'gridAverage',0.2);
%pc = pcdownsample(pc,'nonuniformGridSample',15 ); %30
...
for j = 1:max(L)
    ...
    [gc, dfp,mcp]  = getGricScore(rSqrc,gricParam.sigma, r, d, kc, gricParam.lambda1, gricParam.lambda2);
    if(gc<gp)
        Tmodel(j) = 2;
        T(isInClust) =2;
        Mc(:,j) = mc;
...
figure;
displayClusters(X,T);
save([saveDir,'duomoFacade'],'X','L','T','C3','P','Tmodel','n','epsi','gricParam')
%%
