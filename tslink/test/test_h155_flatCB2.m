...
for modelType = {'mixedFlats'}%{'flat2','flat3','mixedFlats'}
    ...
    saveDir = ['../../../Results/h155/CB2/',dt,'/',modelType,'/trial',id,'/'];
    ...
    nEpsis = 3;
    kappaSigma = linspace(7,9,nEpsis);
    maxIter = 1;
    kappa = 10;
    epsiNfa = 1;
    gricParam.lambda1 = 2;%0.01;
    gricParam.lambda2 = 5;%0.1;
    %%%
    %% view pairs names
    
    data_folder = '../../../Data/h155/';
    addpath(genpath(data_folder));
    temp = load([data_folder,'nameList/nameCB2.mat'],'nameCB2');
    nameDataset = temp.nameCB2;
    ...
    for s = 1:nSeq
        ...
        for iter = 1:maxIter
            ...
            optsSampling.sampling = 'nearest';%'localized';
    ...
    if(o_dumpTestResults)
        ...
        for i = 1:numel(meVec)
            ...
            writecell({'ME std'}, filename,'Sheet',nameSheet,'Range','A83');
            writematrix(kappaSigma,filename,'Sheet',nameSheet,'Range','B83');
            writecell([nameDataset;{'mean'};{'median'}], filename,'Sheet',nameSheet,'Range','A84');
            writematrix([stdMe;mean(stdMe);median(stdMe)],filename,'Sheet',nameSheet,'Range','B84');
            % time
            writecell({'time'}, filename,'Sheet',nameSheet,'Range','A163');
            writematrix(kappaSigma,filename,'Sheet',nameSheet,'Range','B163');
            writecell([nameDataset;{'mean'};{'median'}], filename,'Sheet',nameSheet,'Range','A164');
            writematrix([meanMe;mean(meanMe);median(meanMe)],filename,'Sheet',nameSheet,'Range','B164');
        end
    end
end
%%
