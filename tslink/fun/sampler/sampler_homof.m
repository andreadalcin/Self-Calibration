...
switch voting_id
    ...
    case 4
        ...
        theta =  2.5;
        thresh = epsi.*ones(1,size(R,2));
        for j = 1:size(R,2)
            resid = R(R(:,j)<epsi,j);
            location = nanmedian(resid);
            scale = theta/0.6745 * nanmedian(abs(resid-location));
            thresh(j) = min(scale + location, epsi);
        end
        threshMat = repmat(thresh,size(R,1),1);
        % vote
        heta = 0.05; % minumum preference for residual equal to epsilon
        sigma2 =  -threshMat.^2/log(heta);
        V = exp(-(R.^2)./(sigma2));
    ...
    case 5
        % sigmoid
        a = 1-eps;
        k = log((1+a)/(1-a))*(1/epsi);
        V = 2./(1+exp(k*(R-epsi))) -1;
        P = zeros(size(R));
        I = R< epsi;
        P(I) = V(I);
